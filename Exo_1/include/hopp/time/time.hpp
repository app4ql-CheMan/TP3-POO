...
namespace hopp
{
	...
	public:
		...
		/// @copydoc hopp::time::nanoseconds() const
		template <class T = double>
		T ns() const { return nanoseconds<T>(); }
		
		/// @copydoc hopp::time::microseconds() const
		template <class T = double>
		T us() const { return microseconds<T>(); }
		
		/// @copydoc hopp::time::milliseconds() const
		template <class T = double>
		T ms() const { return milliseconds<T>(); }
		
		/// @copydoc hopp::time::seconds() const
		template <class T = double>
		T s() const { return seconds<T>(); }
		
		/// @copydoc hopp::time::minutes() const
		template <class T = double>
		T min() const { return minutes<T>(); }
		
		/// @copydoc hopp::time::hours() const
		template <class T = double>
		T h() const { return hours<T>(); }
	};
}
